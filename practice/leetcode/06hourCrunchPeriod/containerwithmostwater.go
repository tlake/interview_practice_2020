/*
Given n non-negative integers a1, a2, ..., an , where each represents a point at
coordinate (i, ai). n vertical lines are drawn such that the two endpoints of
line i is at (i, ai) and (i, 0). Find two lines, which together with x-axis forms a
container, such that the container contains the most water.

Note: You may not slant the container and n is at least 2.



The above vertical lines are represented by array [1,8,6,2,5,4,8,3,7]. In this case, the
max area of water (blue section) the container can contain is 49.



Example:

Input: [1,8,6,2,5,4,8,3,7]
Output: 49
*/

func maxArea(height []int) int {
	// x range is 0, len(height)

	type Coord struct {
		X int
		Y int
	}

	coords := make([]Coord, len(height))

	for i, y := range height {
		coord := coords[i]
		coord.X, coord.Y = i, y
	}

	greatestArea := 0

	for ia, _ := range coords {
		for ib := ia; ib < len(coords); ib++ {
			A, B := coords[ia], coords[ib]
			// area: (Y2-Y1)*(X2-X1)
			if area := (B.Y - A.Y) * (B.X - A.X); area > greatestArea {
				greatestArea = area
			}
		}
	}

	return greatestArea
}
